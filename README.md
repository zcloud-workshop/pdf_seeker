...
### PDF 查看器
- **多标签页**：同时打开多个 PDF，标签栏切换，各标签独立保留滚动位置/缩放（同文件去重防止就地编辑冲突）
- **书签管理**：侧栏查看/编辑 PDF 书签（新增、重命名、删除、排序、升降级），写入真实 /Outlines，任意阅读器可见，支持单步撤销
- 连续滚动浏览，支持触控板/鼠标滚轮缩放
- 暗色模式、全屏模式
...
| 工具 | 说明 |
|------|------|
| 合并 (Merge) | 将多个 PDF 文件按顺序合并为一个，支持整文件夹批量导入 |
| 拆分 (Split) | 按页码范围将 PDF 拆分为多个文件 |
| 旋转 (Rotate) | 按 90°/180°/270° 旋转页面，支持缩略图预览 |
| 页面排序 (Reorder) | 拖拽缩略图调整页面顺序，支持重置与保存 |
...
| 图片转 PDF | 将多张图片（JPG/PNG）合并转换为 PDF，支持文件夹批量导入 |
...
| 页面裁剪 | 在预览画布拖拽选区裁剪页面（当前页或全部页），支持撤销/重做 |
| 批注 | 写入真实 PDF 批注对象：高亮、下划线（拖拽选区）、便签（点击 + 文字），任意阅读器可见 |
| 表单填写 | 读取 AcroForm 表单字段（文本/复选/单选/下拉）并填写另存 |
| 替换文本 | 搜索文本列出全部匹配，勾选后覆盖式替换（原位覆盖 + 新文本），支持撤销 |
| PDF 安全 | RC4 128-bit 密码保护加密（用户/所有者密码 + 权限位）与解密另存 |
| 表格提取 | 检测指定页面的表格并导出为 CSV（基于文本坐标，适用于文本型 PDF） |
| 插件 | 运行外部命令行插件处理当前 PDF（manifest 注册，见 [docs/plugins.md](docs/plugins.md)） |
...
- 拖拽完成后自动重新渲染预览
- **就地编辑 + 撤销/重做**：编辑直接写入当前文件，`Ctrl/Cmd+Z` 撤销、`Ctrl/Cmd+Shift+Z` 重做（内存快照，最多 30 步）

### PDF 对比（Diff）
- 选择 A/B 两个 PDF：**文本差异**（逐页行级 +/- 着色、页级汇总、仅显示差异页过滤）与**并排视觉对比**（同步滚动）双模式
- 差异页可一键跳转到并排视图对应位置
...
- **配置云同步**：一键/自动备份 config.toml 到 S3（`config-backup/` 前缀），按版本恢复，跨设备迁移设置

### 自动更新
- 基于 Tauri updater 插件，更新源为 GitHub Releases 的 `latest.json`（签名验证）
- 设置页可开关「启动时自动检查更新」，也可立即检查；下载安装后可一键重启
...
- `tauri-plugin-updater` / `tauri-plugin-process 2` — 自动更新与重启
...
### 短期（v0.2）
- [x] 表格提取功能（Table Extraction）
- [x] 深色模式下的水印/签名预览优化
- [x] 编辑工具撤销/重做（Undo/Redo）
- [x] PDF 页面拖拽排序
- [x] 批量文件处理（文件夹导入）
...
### 长期（v0.4 完成）
- [x] 云端同步（S3 自动备份/恢复配置文件）
- [x] 多标签页同时打开多个 PDF
- [x] PDF 书签管理
- [x] PDF 对比（Diff）
- [x] 插件系统（外部命令插件 v1，见 [docs/plugins.md](docs/plugins.md)）
- [x] 自动更新（Tauri updater 插件）

## 已知限制

- 替换文本为覆盖式实现：原文字用白色矩形覆盖后叠加新文本（Helvetica 同字号），无法复用原文的子集字体；深色/图片背景上的替换会留下白色覆盖块；不支持跨行与旋转文本匹配
- 编辑工具为就地写入：撤销/重做基于内存快照（最多 30 步），切换文件或退出应用后历史清空，重要文件建议先备份
- 表格提取基于文本坐标启发式聚类，对扫描件（无文字层）或复杂合并单元格表格效果有限
- OCR 识别依赖外部 Tesseract 引擎，未安装时仅使用 pdfjs 内置文字提取
- 加密采用 RC4 128-bit 标准安全 handler（V2/R3，兼容全部阅读器，但算法较旧）；解密仅支持同标准（不支持 AES-256 或公钥加密的文件）
- S3 存储操作依赖网络连接，离线时不可用
- PDF 对比的文本模式仅比较文字层：扫描件（无文字层）只能用并排视觉模式；页按序号对齐，插入/删除页会导致后续错位
- 插件为你本人放入本机的可信外部命令，以当前用户权限运行，无沙箱隔离（v1 范围）
- 配置云同步备份包含明文 S3 密钥，请务必使用私有存储桶；恢复会整体覆盖本地设置（含 S3 连接配置本身）
- 书签编辑为就地写入，撤销仅单步（Cmd+Z 恢复最近一次写入前）
- 自动更新要求 GitHub Release 携带签名产物（`latest.json` + `.sig`）；维护者需配置 `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` 两个仓库 Secrets。免签分发下 macOS 首次安装仍需 `xattr` 解除隔离，之后的版本更新可走应用内更新
